...
//go:generate bash -c "cd ../../xend-contracts && git checkout 6858d2716f255d815d250f90d7c8ac1ed86b2353"
//go:generate bash -c "docker run --name go-xend-contracts-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../xend-contracts:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-xend-contracts-compiler go-xend-contracts-compiler-image && docker rm go-xend-contracts-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../xend-contracts:/src -w /src go-xend-contracts-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../xend-contracts:/src -w /src go-xend-contracts-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../xend-contracts && git checkout -- truffle-config.js; docker rmi go-xend-contracts-compiler-image"
